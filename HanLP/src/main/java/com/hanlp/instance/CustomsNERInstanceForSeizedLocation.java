...
/**
 * Title: 
 * Description: 查获地址<SeizedLocation> 命名实体 特征提取
 * Copyright: 2020 北京拓尔思信息技术股份有限公司 版权所有.保留所有权
 * Company:北京拓尔思信息技术股份有限公司(TRS)
 * Project: SpringBootDemo
 * Author: 王杰
 * Create Time:2020/3/2 09:25
 */
public class CustomsNERInstanceForSeizedLocation extends NERInstance {

	public CustomsNERInstanceForSeizedLocation(String[] wordArray, String[] posArray, String[] nerArray, NERTagSet
			tagSet, FeatureMap featureMap) {
		super(wordArray, posArray, nerArray, tagSet, featureMap);
	}

	public CustomsNERInstanceForSeizedLocation(String[] wordArray, String[] posArray, FeatureMap featureMap) {
	...
	protected int[] extractFeature(String[] wordArray, String[] posArray, FeatureMap featureMap, int position) {
		...
		// 前面是介词
		String customsPreWord = position >= 1 ? (Objects.equals("在", wordArray[position - 1]) ? "PY" : "PN") : "PN";
		// 后面是动词
		String customsNextPos = position <= posArray.length - 2 ? ((Objects.equals("v", posArray[position + 1])) ? "YV" : "NV") : "NV";
		...
		// 考虑词性问题
		addFeatureThenClear(sb.append(customsPreWord).append('A'), featVec, featureMap);
		addFeatureThenClear(sb.append(customsPreWord).append('B'), featVec, featureMap);
		addFeatureThenClear(sb.append(customsPreWord).append('C'), featVec, featureMap);
		addFeatureThenClear(sb.append(customsPreWord).append('D'), featVec, featureMap);
